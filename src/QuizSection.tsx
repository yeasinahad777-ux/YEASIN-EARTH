...
export default function QuizSection({ onExit }: QuizSectionProps) {
	...
	const startQuiz = (selectedCategory: QuizCategory) => {
		...
		if (selectedCategory === "currency") {
			validCountries = tempCountries.filter(
				(c) => currenciesMap[c.code] && currenciesMap[c.code] !== "অজানা",
			);
		}
		...
		for (let i = 0; i < 10; i++) {
			...
			while (options.length < 4) {
				...
				if (selectedCategory === "capital") wrongAns = randCountry.capital;
					...
					wrongAns = randCountry.neighbors;
				else if (selectedCategory === "currency")
					wrongAns = currenciesMap[randCountry.code];

				if (
					!options.includes(wrongAns) &&
					wrongAns !== "নেই, নেই" &&
					wrongAns !== "অজানা" &&
					wrongAns
				) {
		...
		return (
			<div className="max-w-2xl mx-auto bg-[#0a0a12] p-8 rounded-3xl border border-white/10 shadow-2xl text-center mt-10">
				<h2 className="text-3xl font-black text-white mb-2">
					কুইজের বিষয় নির্বাচন করুন
				</h2>
				<p className="text-gray-400 mb-8 font-medium">
					আপনি কোন বিষয়ের উপর পরীক্ষা দিতে চান তা নিচে থেকে বেছে নিন
				</p>
				...
				<div className="flex flex-col gap-4">
					...
					>
						...
						<div>
							...
							<p className="text-sm text-gray-400 font-medium">
								বিভিন্ন দেশের রাজধানীর নাম নিয়ে কুইজ
							</p>
						</div>
					</button>
					...
					>
						...
						<div>
							...
							<p className="text-sm text-gray-400 font-medium">
								কোন দেশের আশেপাশে কোন দেশ আছে তা নিয়ে কুইজ
							</p>
						</div>
					</button>
					...
					>
						...
						<div>
							<h3 className="text-xl font-bold text-white mb-1">
								দেশের মুদ্রা (Currency)
							</h3>
							<p className="text-sm text-gray-400 font-medium">
								{isLoadingCurrencies
									? "মুদ্রার তথ্য লোড হচ্ছে..."
									: "বিভিন্ন দেশের মুদ্রার নাম নিয়ে কুইজ"}
							</p>
						</div>
					</button>
				</div>
		...
		return (
			<div className="max-w-2xl mx-auto bg-[#0a0a12] p-10 rounded-3xl border border-white/10 shadow-2xl text-center mt-10">
				...
				<div className="text-2xl font-bold text-gray-400 mb-8">
					১০ এর মধ্যে আপনি পেয়েছেন:{" "}
					<span className="text-blue-500 text-6xl block mt-6 mb-4 font-black">
						{score}
					</span>
				</div>
	...
	return (
		<div className="max-w-2xl mx-auto bg-[#0a0a12] p-6 md:p-10 rounded-[32px] border border-white/10 shadow-2xl mt-4 md:mt-10 overflow-hidden relative">
			...
			<div className="flex justify-between items-center mb-8 font-bold border-b border-white/5 pb-4 mt-2">
				<span className="text-gray-400 bg-white/5 px-4 py-1.5 rounded-full border border-white/10">
					প্রশ্ন: <span className="text-white">{currentQIndex + 1}</span>/১০
				</span>
				<span className="text-emerald-400 bg-emerald-500/10 px-4 py-1.5 rounded-full border border-emerald-500/20">
					স্কোর: <span className="font-black text-emerald-300">{score}</span>
				</span>
			</div>

			<div className="w-full h-40 md:h-52 mb-8 rounded-2xl overflow-hidden border border-white/10 relative bg-gradient-to-t from-[#030712] to-blue-900/10 pointer-events-none">
				<GlobeViz focusCountryCode={qData.targetCode} />
				<div className="absolute inset-0 bg-[#0a0a12]/20 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] z-10 pointer-events-none"></div>
			</div>

			<h3 className="text-2xl md:text-3xl font-black text-white mb-10 leading-relaxed tracking-tight">
				{qData.question}
			</h3>
			...
			<div className="flex flex-col gap-4 mb-10">
				{qData.options.map((opt, idx) => {
					let btnClass =
						"w-full p-5 text-left text-lg bg-[#030712] text-white border border-white/10 rounded-2xl transition-all shadow-sm font-medium ";

					if (selectedAnswer) {
						if (opt === qData.answer) {
							btnClass +=
								"bg-emerald-600 border-none shadow-emerald-500/30 ring-2 ring-emerald-400 font-bold";
						} else if (opt === selectedAnswer) {
							btnClass +=
								"bg-rose-600 border-none shadow-rose-500/30 font-bold";
						} else {
							btnClass += "opacity-40 cursor-not-allowed";
						}
					} else {
						btnClass +=
							"hover:bg-white/10 hover:border-blue-500/50 hover:shadow-lg cursor-pointer active:scale-[0.98]";
					}
